...
This design system establishes a disciplined, trustworthy, and pragmatic mobile-first environment tailored for agricultural producers, trade cooperatives, and commodity aggregators. The visual narrative balances institutional financial authority with grounded agricultural utility. It projects stability, fiscal clarity, and operational confidence without decorative distraction.

The style rejects decorative trends such as translucent glass, floating multi-layered cards, and neon accent gradients. Instead, it deploys a clean modern structure influenced by high-density financial data terminals, warm agricultural ledgers, and utilitarian editorial typography. 

Key attributes:

- **Pragmatic and Honest:** Every line, numeral, and container exists to inform yield, price movement, and profit margin.

- **High-Density Legibility:** Designed for variable outdoor lighting conditions, glanceable ledger scanning, and rapid data entry under field conditions.

- **Rooted Stability:** Deep botanical greens anchor financial figures against paper-toned backgrounds, replacing cold fintech blues with organic reliability.
...
The type system blends structural geometric precision `Manrope`) with the open counterforms and high legibility of `Public Sans`. All numerical values, prices, metrics, and currency units must utilize tabular figures `tnum`) and slashed zeros `zero`) to ensure strict vertical alignment across financial lists and balances.

Rules for Vietnamese commodity and currency notation:

- Currency symbol `₫` or `VND`) follows the numeric value separated by a non-breaking space (e.g., `125,500,000 ₫`).

- Unit denominator notation `/kg`, `/tấn`, `/bao`) must scale down to `label-md` or `body-sm` in a medium weight to maintain proportional balance against large values.

- Never use stylized or cursive serif fonts for metrics. Financial balances require instant readability under glare and high-velocity field scrolling.

## Layout &amp; Spacing

The layout is built around a predictable mobile vertical rhythm, adhering to a 4px baseline sub-grid with an 8px primary component step. 

### Screen Grid &amp; Ergonomics

- **Mobile Grid:** Single-column content stack with `16px` outer margins and `12px` inner component gutters.

- **Card Padding:** Standardized to `16px` vertically and `16px` horizontally. High-density balance cards expand to `20px` internal padding.

- **Touch Targets:** Strictly enforcement of a `44px` `2.75rem`) minimum interactive height across all controls, buttons, row navigators, and filter chips.

- **Bottom Navigation Anchor:** Permanent `64px` height baseline with safe area inset at the screen bottom, ensuring thumbs effortlessly reach primary switches: Markets, Inventory, Ledger, and Profile.

- **Data Densification:** On tablet or larger screens, content conforms to a centered 640px viewport or expands to a symmetrical 2-column master-detail grid with 24px margins.
...
The design system adopts a soft, disciplined shape language `roundedness: 1`). Elements are clean, measured, and approachable without appearing playful or juvenile.

- **Base Radius (4px / `0.25rem`):** Applied to data tags, small unit indicators, checkboxes, and nested financial line indicators.

- **Container Radius (8px / `0.5rem`):** Applied to input fields, buttons, standard card containers, and bottom navigation items.

- **Structural Sheet Radius (12px / `0.75rem`):** Applied strictly to top corners of bottom modal sheets and primary portfolio summary blocks.

- **Pills / Circles:** Reserved exclusively for status indicators, circular supplier avatars, and directional trend icons.
